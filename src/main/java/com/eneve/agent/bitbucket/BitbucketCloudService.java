...
public class BitbucketCloudService {
    ...
    /**
     * List all repository slugs in a Bitbucket workspace, paginating through all results.
     */
    public List<String> listRepositories(String workspace) {
        List<String> slugs = new ArrayList<>();
        String path = "/repositories/" + workspace + "?pagelen=100";

        while (path != null) {
            String responseBody = getAndReturn(path, "list repos for workspace " + workspace);
            try {
                JsonNode root = objectMapper.readTree(responseBody);
                JsonNode values = root.path("values");
                if (values.isArray()) {
        ...
        LOG.infof("Listed %d repositories in workspace '%s'", slugs.size(), workspace);
        return slugs;
    }
