 ...
 */
@ApplicationScoped
public class ReviewFeatureHandler implements JobHandler {

    private static final Logger LOG = Logger.getLogger(ReviewFeatureHandler.class);
    ...
    public void handle(JobRecord job) {
        ...
        if (req.roadmapId() != null && overrideStore.isOverridden(req.roadmapId(), req.issueKey())) {
            LOG.infof("ReviewFeature %s: item is overridden, skipping Claude call", req.issueKey());
        ...
        reviewStore.upsert(
                req.roadmapId(), req.issueKey(), "FEATURE", issueSummary,
        ...
        job.setStatus(JobStatus.SUCCESS);
        job.setSummary("Feature " + req.issueKey() + " reviewed: readiness=" + readinessScore
                + " complexity=" + complexityScore);
        jobStore.archive(job);
        LOG.infof("ReviewFeature %s: readiness=%d label=%s complexity=%d",
    ...
    private String callClaude(String prompt, String jobId) {
        ...
        try {
            ...
            LOG.errorf("ReviewFeatureHandler: Claude call failed for job %s: %s", jobId, e.getMessage());
            return null;
        }
    }
    ...
    private void fail(JobRecord job, String message) {
        LOG.errorf("ReviewFeatureHandler: %s", message);
