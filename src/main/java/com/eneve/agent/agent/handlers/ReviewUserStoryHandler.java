 ...
 */
@ApplicationScoped
public class ReviewUserStoryHandler implements JobHandler {

    private static final Logger LOG = Logger.getLogger(ReviewUserStoryHandler.class);
    ...
    public void handle(JobRecord job) {
        ...
        if (req.roadmapId() != null && overrideStore.isOverridden(req.roadmapId(), req.issueKey())) {
            LOG.infof("ReviewUserStory %s: item is overridden, skipping Claude call", req.issueKey());
        ...
        String context = contextBuilder.buildUserStoryContext(
                req.issueKey(), req.parentKey(), req.grandparentKey());
        String prompt = promptTemplates.resolve("review-userstory", Map.of("jira_context", context));
        ...
        reviewStore.upsert(
                req.roadmapId(), req.issueKey(), "USERSTORY", issueSummary,
        ...
        job.setStatus(JobStatus.SUCCESS);
        job.setSummary("UserStory " + req.issueKey() + " reviewed: readiness=" + readinessScore
                + " complexity=" + complexityScore);
        jobStore.archive(job);
        LOG.infof("ReviewUserStory %s: readiness=%d label=%s complexity=%d",
    ...
    private String callClaude(String prompt, String jobId) {
        ...
        try {
            ...
            LOG.errorf("ReviewUserStoryHandler: Claude call failed for job %s: %s", jobId, e.getMessage());
            return null;
        }
    }
    ...
    private void fail(JobRecord job, String message) {
        LOG.errorf("ReviewUserStoryHandler: %s", message);
